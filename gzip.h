...
#define GZIP_NAMESPACE_BEGIN    namespace gzip {  
...
inline std::string compress(const std::string& data, int level = -1) {
    std::string out;
    if (!compress(data, out, level)) {
        out.clear();
    }
    return out;
}
...
inline std::string uncompress(const std::string& compressedData) {
    std::string out;
    if (!uncompress(compressedData, out)) {
        out.clear();
    }
    return out;
}
