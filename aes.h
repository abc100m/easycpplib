/*
* AES 加解密, 支持ECB和CBC模式
* 来自 https://github.com/kkAyataka/plusaes
* 另一个纯C的也不错: https://github.com/kokke/tiny-AES128-C (这个看issue还有不少bug)
*/
...
#define AES_NAMESPACE_BEGIN  namespace aes {
#define AES_NAMESPACE_END    }
...
//用于取到16的倍数. 是这个库的特有算法, @len是16的倍数也要多加16个字节
size_t inline round_up_16(size_t len)
{
    return len + (16 - len % 16);
}

std::string encrypt_ecb(
        const unsigned char * data,
        const unsigned long data_size,
        const unsigned char * key,
        const unsigned long key_size,
...
{
    unsigned long encrypted_size = data_size;
    if (pads) {
        encrypted_size = round_up_16(encrypted_size);
    }
    std::string out(encrypted_size, '\0');
    auto err = encrypt_ecb(data, data_size, key, key_size, (unsigned char*)out.data(), out.size(), pads);
    if (err != ERROR_OK) {
        return std::string();
    }
    return out;
...
//The @key length must be 16 (128-bit), 24 (192-bit) or 32 (256-bit).  padding use PKCS#7 
inline std::string encrypt_ecb(
        const std::string& data,
        const std::string& key,
        bool pads = true
      )
{
    return encrypt_ecb((const unsigned char*)data.c_str(), data.size(), (const unsigned char*)key.c_str(), key.size(), pads);
}

std::string decrypt_ecb(
        const unsigned char * data,
        const unsigned long data_size,
        const unsigned char * key,
        const unsigned long key_size
       )
{
    ...
    auto err = decrypt_ecb(data, data_size, key, key_size, (unsigned char*)out.data(), out.size(), &padded);
    if (err != ERROR_OK) {
        return std::string();
    }
    return std::string(out.begin(), out.end() - padded);
}

inline std::string decrypt_ecb(const std::string& data, const std::string& key) 
{
    return decrypt_ecb((const unsigned char*)data.c_str(), data.size(), (const unsigned char*)key.c_str(), key.size());
}

std::string encrypt_cbc(
        const unsigned char * data,
        const unsigned long data_size,
        const unsigned char * key,
        const unsigned long key_size,
       ...
       )
{
    ...
    std::string out(encrypted_size, '\0');
    auto err = encrypt_cbc(data, data_size, key, key_size, iv, (unsigned char*)out.data(), out.size(), pads);
    if (err != ERROR_OK) {
        return std::string();
    }
    return out;
...
//The @key length must be 16 (128-bit), 24 (192-bit) or 32 (256-bit). padding use PKCS#7 
inline std::string encrypt_cbc(
        const std::string& data,
        const std::string& key,
        const unsigned char (* iv)[16],
        const bool pads = true
       )
{
    return encrypt_cbc((const unsigned char*)data.c_str(), data.size(), (const unsigned char*)key.c_str(), key.size(), iv, pads);
}

std::string decrypt_cbc(
        const unsigned char * data,
        const unsigned long data_size,
        const unsigned char * key,
        const unsigned long key_size,
       ...
       )
{
    unsigned long padded = 0;
    std::string out(data_size, '\0');
    auto err = decrypt_cbc(data, data_size, key, key_size, iv, (unsigned char*)out.data(), out.size(), &padded);
    if (err != ERROR_OK) {
        return std::string();
    }
    return std::string(out.begin(), out.end() - padded);
...
inline std::string decrypt_cbc(
        const std::string &data,
        const std::string &key,
        const unsigned char (* iv)[16]
       )
{
    return decrypt_cbc((const unsigned char*)data.c_str(), data.size(), (const unsigned char*)key.c_str(), key.size(), iv);
}

#if 0
void foo_test()
{
    string bdata = "asdfasdf";
    string aes_key = "my_private_key12"; //必须是16，24，32字节
    auto aes_x1 = aes::encrypt_ecb(bdata, aes_key);
    auto aes_x2 = aes::decrypt_ecb(aes_x1, aes_key);
    assert(aes_x2 == bdata);
    auto b64_aes_x1 = base64::encode_base64(aes_x1);

    const unsigned char iv[16] = { 0x73, 0xD4, 0xA3, 0x24, 0x73, 0xF4, 0xD6, 0x90, 0xEE, 0xA2, 0x5E, 0xE6, 0xE0, 0x9F, 0xF5, 0x49 };
    const unsigned char iv2[16] = { 0x73, 0xD4, 0xA3, 0x24, 0x73, 0xF4, 0xD6, 0x90, 0xEE, 0xA2, 0x5E, 0xE6, 0xE0, 0x9F, 0xF5, 0x49 };
    for (int i = 0; i < 15; ++i) {
        auto aes_y1 = aes::encrypt_cbc(bdata, aes_key, &iv);
        auto decrypt1 = aes::decrypt_cbc(aes_y1, aes_key, &iv2);
        cout << base64::encode_base64(aes_y1) << "\t" << base64::encode_base64(decrypt1) << endl;
    }
}
#endif

AES_NAMESPACE_END // namespace plusaes
